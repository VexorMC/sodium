...
import static dev.vexor.radium.compat.lwjgl3.Pointer.*;
...
public final class MemoryUtil {
    ...
    private static MemoryAllocator ALLOCATOR = new UnsafeAllocator();

    private static boolean CHECKS = false;
    private static void check(long addr) {}
    private static void check(long addr1, long addr) {}
    private static void check(Buffer addr1, long addr) {}
    ...
    static {
        ...
        try {
            MARK = UNSAFE.objectFieldOffset(Buffer.class.getDeclaredField("mark"));
            POSITION = UNSAFE.objectFieldOffset(Buffer.class.getDeclaredField("position"));
            LIMIT = UNSAFE.objectFieldOffset(Buffer.class.getDeclaredField("limit"));
            CAPACITY = UNSAFE.objectFieldOffset(Buffer.class.getDeclaredField("capacity"));
     ...
     *
     * <p>Allocations made through the returned instance will not be tracked for memory leaks, even if {@link } is enabled.
     ...
     * Returns the {@link MemoryAllocator} instance used internally by the explicit memory management API ({@link #memAlloc}, {@link #memFree}, etc).
     *
     * @param tracked whether allocations will be tracked for memory leaks, if {@link } is enabled.
     *
     * @return the {@link MemoryAllocator} instance
     */
    ...
    public static PointerBuffer memCallocPointer(int num) {
        return PointerBuffer.allocateDirect(num);
    }
    ...
    public static void memCopy(ByteBuffer src, ByteBuffer dst) {
        ...
        UNSAFE.copyMemory(memAddress(src), memAddress(dst), src.remaining());
    }
    ...
    public static void memCopy(ShortBuffer src, ShortBuffer dst) {
        ...
        UNSAFE.copyMemory(memAddress(src), memAddress(dst), apiGetBytes(src.remaining(), 1));
    }
    ...
    public static void memCopy(IntBuffer src, IntBuffer dst) {
        ...
        UNSAFE.copyMemory(memAddress(src), memAddress(dst), apiGetBytes(src.remaining(), 2));
    }
    ...
    public static void memCopy(LongBuffer src, LongBuffer dst) {
        ...
        UNSAFE.copyMemory(memAddress(src), memAddress(dst), apiGetBytes(src.remaining(), 3));
    }
    ...
    public static <T extends CustomBuffer<T>> void memCopy(T src, T dst) {
        UNSAFE.copyMemory(memAddress(src), memAddress(dst), Integer.toUnsignedLong(src.remaining()) * src.sizeof());
    }
    ...
    private static String decodeUTF8(long source, int length) {
        if (length <= 0) {
            return "";
        }
    ...
    public static String memUTF8(long address) {
        return decodeUTF8(address, memLengthNT1(address, Integer.MAX_VALUE));
    }
    ...
    public static String memUTF8(long address, int length) {
        return decodeUTF8(address, length);
    }
    ...
    public static String memUTF8(ByteBuffer buffer) {
        return decodeUTF8(memAddress(buffer), buffer.remaining());
    }
    ...
    public static @Nullable String memUTF8Safe(long address) {
        return address == NULL ? null : decodeUTF8(address, memLengthNT1(address, Integer.MAX_VALUE));
    }
    ...
    public static @Nullable String memUTF8Safe(long address, int length) {
        return address == NULL ? null : decodeUTF8(address, length);
    }
    ...
    public static @Nullable String memUTF8Safe(@Nullable ByteBuffer buffer) {
        return buffer == null ? null : decodeUTF8(memAddress(buffer), buffer.remaining());
    }
    ...
    public static String memUTF8(ByteBuffer buffer, int length) {
        return decodeUTF8(memAddress(buffer), length);
    }
    ...
    public static String memUTF8(ByteBuffer buffer, int length, int offset) {
        return decodeUTF8(memAddress(buffer, offset), length);
    }
