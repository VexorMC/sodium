package dev.vexor.radium.compat.mojang.render;
...
public abstract class RenderLayer extends RenderPhase { // Aka: RenderType (Iris)
    private static final RenderLayer SOLID = of("solid", VertexFormats.POSITION_TEXTURE_COLOR_NORMAL, 7, 2097152, true, false, RenderLayer.MultiPhaseParameters.builder().shadeModel(SMOOTH_SHADE_MODEL).lightmap(ENABLE_LIGHTMAP).texture(MIPMAP_BLOCK_ATLAS_TEXTURE).build(true));
    private static final RenderLayer CUTOUT = of("cutout", VertexFormats.POSITION_TEXTURE_COLOR_NORMAL, 7, 131072, true, false, RenderLayer.MultiPhaseParameters.builder().shadeModel(SMOOTH_SHADE_MODEL).lightmap(ENABLE_LIGHTMAP).texture(BLOCK_ATLAS_TEXTURE).alpha(HALF_ALPHA).build(true));
    private static final RenderLayer CUTOUTMIPPED = of("cutoutMipped", VertexFormats.POSITION_TEXTURE_COLOR_NORMAL, 7, 131072, true, false, RenderLayer.MultiPhaseParameters.builder().shadeModel(SMOOTH_SHADE_MODEL).lightmap(ENABLE_LIGHTMAP).texture(BLOCK_ATLAS_TEXTURE).alpha(HALF_ALPHA).build(true));
    private static final RenderLayer TRANSLUCENT = of("translucent", VertexFormats.POSITION_TEXTURE_COLOR_NORMAL, 7, 262144, true, true, createTranslucentPhaseData());
    ...
    public static RenderLayer getOutline(Identifier texture, RenderPhase.Cull cull) {
        return of("outline", VertexFormats.POSITION_TEXTURE_COLOR, 7, 256, RenderLayer.MultiPhaseParameters.builder().texture(new RenderPhase.Texture(texture, false, false)).cull(cull).depthTest(ALWAYS_DEPTH_TEST).alpha(ONE_TENTH_ALPHA).fog(NO_FOG).target(OUTLINE_TARGET).build(RenderLayer.OutlineMode.IS_OUTLINE));
    }
