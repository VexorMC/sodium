...
public class OptionPageScrollFrame extends AbstractFrame {
    ...
    private void renderOptionTooltip(ControlElement<?> element) {
        ...
        List<String> tooltip = new ArrayList<>(MinecraftClient.getInstance().textRenderer.wrapLines(option.getTooltip().asFormattedString(), boxWidth - (textPadding * 2)));

        OptionImpact impact = option.getImpact();

        if (impact != null) {
            tooltip.add(new TranslatableText("sodium.options.performance_impact_string").setStyle(new Style().setFormatting(Formatting.GRAY)).asFormattedString());
        }
