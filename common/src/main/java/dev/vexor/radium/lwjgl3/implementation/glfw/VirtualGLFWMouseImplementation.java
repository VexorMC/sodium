...
public class VirtualGLFWMouseImplementation implements MouseImplementation {
    ...
    private boolean isValidScreen() {
        ...
        return s instanceof InventoryScreen || s instanceof ChatScreen || s instanceof BookEditScreen
                || s instanceof AnvilScreen || s instanceof BeaconScreen
                || s instanceof BrewingStandScreen || s instanceof ChestScreen
                || s instanceof CraftingTableScreen || s instanceof CreativeInventoryScreen
                || s instanceof DispenserScreen || s instanceof EnchantingScreen
                || s instanceof FurnaceScreen || s instanceof HopperScreen
                || s instanceof VillagerTradingScreen || s instanceof SignEditScreen;
    }
    ...
    private void drawTexture(double x, double y, double width, double height, double textureWidth, double textureHeight) {
        ...
        bufferBuilder.vertex(x, y + height, z).texture(0, height * o).next();
        bufferBuilder.vertex(x + width, y + height, z).texture(width * n, height * o).next();
        bufferBuilder.vertex(x + width, y, z).texture(width * n, 0).next();
        bufferBuilder.vertex(x, y, z).texture(0, 0).next();
        tessellator.draw();
    }
    ...
    private void loadCursor() {
        ...
        for (int i = 0; i < images.length; i++) {
            ...
            int id = TextureUtil.getTexLevelParameter();
            images[i] = id;
            TextureUtil.prepareImage(id, (int) c.width, (int) c.height);
            TextureUtil.method_5861(id, c.getImage(), (int) c.width, (int) c.height);
        }
    }
    ...
    private static class SystemCursor {
        ...
        private InputStream getArrowCursor() throws IOException {
            ...
            try {
                return MinecraftClient.getInstance().getResourceManager().getResource(new ModelIdentifier("virtual_cursor", "default")).getInputStream();
