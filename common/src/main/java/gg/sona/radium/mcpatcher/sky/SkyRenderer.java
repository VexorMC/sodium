package gg.sona.radium.mcpatcher.sky;
...
import gg.sona.radium.mcpatcher.MCLogger;
...
import gg.sona.radium.mcpatcher.mal.resource.BlendMethod;
import gg.sona.radium.mcpatcher.mal.resource.GLAPI;
import gg.sona.radium.mcpatcher.mal.resource.PropertiesFile;
import gg.sona.radium.mcpatcher.mal.resource.TexturePackAPI;
...
public class SkyRenderer {
    ...
    private static final boolean enable = SodiumClientMod.options().quality.betterSkies;
    private static final boolean unloadTextures = false;
        ...
        TexturePackChangeHandler.register(new TexturePackChangeHandler(MCPatcherUtils.BETTER_SKIES, 2) {
            ...
            public void afterChange() {
                if (enable) {
                    ...
                    if (world != null) {
                        getWorldEntry(world.dimension.getType());
                    }
                }
    ...
    public static void setup(World world, float partialTick, float celestialAngle) {
        if (TexturePackAPI.isDefaultTexturePack()) {
            ...
            int worldType = MinecraftClient.getInstance().world.dimension.getType();
    ...
    private static class WorldEntry {
        ...
        private final Map<Identifier, Layer> objects = new HashMap<>();
        ...
        private void loadSkies() {
            for (int i = -1;; i++) {
                ...
                Identifier resource = TexturePackAPI.newMCPatcherResourceLocation(path);
        ...
        private void loadCelestialObject(String objName) {
            Identifier textureName = new Identifier("textures" + "/environment/" + objName + ".png");
        ...
        void renderAll() {
            if (unloadTextures) {
                Set<Identifier> texturesNeeded = new HashSet<>();
                for (Layer layer : skies) {
                    if (layer.prepare()) {
                        texturesNeeded.add(layer.texture);
                    }
                }
                Set<Identifier> texturesToUnload = new HashSet<>(textures);
    ...
    private static class Layer {
        ...
        private boolean readTexture() {
            texture = properties.getIdentifier(
                    "source",
                    properties.toString()
        ...
        private static void drawTile(int tile) {
            ...
            bufferBuilder.begin(GL11.GL_QUADS, net.minecraft.client.render.VertexFormats.POSITION_TEXTURE);
            bufferBuilder.vertex(-SKY_DISTANCE, -SKY_DISTANCE, -SKY_DISTANCE).texture(tileX, tileY).next();
            bufferBuilder.vertex(-SKY_DISTANCE, -SKY_DISTANCE, SKY_DISTANCE).texture(tileX, tileY + 0.5).next();
            bufferBuilder.vertex(SKY_DISTANCE, -SKY_DISTANCE, SKY_DISTANCE).texture(tileX + 1.0 / 3.0, tileY + 0.5).next();
            bufferBuilder.vertex(SKY_DISTANCE, -SKY_DISTANCE, -SKY_DISTANCE).texture(tileX + 1.0 / 3.0, tileY).next();
