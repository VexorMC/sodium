...
public class OptionPageFrame extends AbstractFrame {
    ...
    public void buildFrame() {
        ...
        for (OptionGroup group : this.page.getGroups()) {
            ...
            for (Option<?> option : group.getOptions()) {
                ...
                final ControlElement<?> element = control.createElement(new Dim2i(this.dim.getOriginX(), this.dim.getOriginY() + y, this.dim.getWidth(), 18));
    ...
    private void renderOptionTooltip(ControlElement<?> element) {
        ...
        final List<String> tooltip = new ArrayList<>(fontRenderer.wrapLines(option.getTooltip().asFormattedString(), boxWidth - (textPadding * 2)));

        final OptionImpact impact = option.getImpact();

        if (impact != null) {
            tooltip.add(Formatting.GRAY + I18n.translate("sodium.options.performance_impact_string", impact.getLocalizedName().asFormattedString()));
        }
        ...
        for (int i = 0; i < tooltip.size(); i++) {
            fontRenderer.draw(tooltip.get(i), boxX + textPadding, boxY + textPadding + (i * 12), 0xFFFFFFFF);
        }
    }
