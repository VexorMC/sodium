...
public class ChunkRenderManager<T extends ChunkGraphicsState> implements ChunkStatusListener, SwappableChunkRenderManager {
    ...
    private void setup(float ticks) {
        ...
        if (Iris.getCurrentPack().isEmpty() && SodiumClientMod.options().advanced.useFogOcclusion) {
    ...
    private void addChunk(ChunkRenderContainer<T> render) {
        if (ShadowRenderingState.areShadowsCurrentlyBeingRendered()) {
            // Do not schedule rebuilds or sorts during shadow pass
            if (!render.isEmpty()) {
    ...
    public ChunkRenderContainer<T> getRender(int x, int y, int z) {
        ChunkRenderColumn<T> column = this.columns.get(ChunkPos.getIdFromCoords(x, z));
    ...
    private void loadChunk(int x, int z) {
        ...
        if ((prev = columns.put(ChunkPos.getIdFromCoords(x, z), column)) != null) {
            unloadSections(prev);
        }

        connectNeighborColumns(column);
        loadSections(column);
    ...
    private void unloadChunk(int x, int z) {
        ChunkRenderColumn<T> column = columns.remove(ChunkPos.getIdFromCoords(x, z));

        if (column == null) {
            return;
        }

        disconnectNeighborColumns(column);
        unloadSections(column);
    ...
    private void loadSections(ChunkRenderColumn<T> column) {
        ...
        for (int y = 0; y < 16; y++) {
            ChunkRenderContainer<T> render = createChunkRender(column, x, y, z);
            column.setRender(y, render);

            culler.onSectionLoaded(x, y, z, render.getId());
        }
    }
    ...
    private void unloadSections(ChunkRenderColumn<T> column) {
        ...
        for (int y = 0; y < 16; y++) {
            ...
            if (render != null) {
                unloadQueue.enqueue(render);
                renders.remove(render.getId());
            }

            culler.onSectionUnloaded(x, y, z);
        }
    }
    ...
    private ChunkRenderColumn<T> getColumn(int x, int z) {
        return this.columns.get(ChunkPos.getIdFromCoords(x, z));
    }
    ...
    private ChunkRenderContainer<T> createChunkRender(ChunkRenderColumn<T> column, int x, int y, int z) {
        ...
        final ChunkSection array = this.world.getChunk(x, z).getBlockStorage()[y];
    ...
    public void updateChunks() {
        if (ShadowRenderingState.areShadowsCurrentlyBeingRendered()) {
            return; // Prevent chunk rebuilds during shadow rendering
        }
    ...
    public void iris$swapVisibilityState() {
        ChunkRenderList<T>[] tmpLists = chunkRenderLists;
        chunkRenderLists = chunkRenderListsSwap;
        chunkRenderListsSwap = tmpLists;

        ObjectList<ChunkRenderContainer<T>> tmpTick = tickableChunks;
        tickableChunks = tickableChunksSwap;
        tickableChunksSwap = tmpTick;

        ObjectList<BlockEntity> tmpBlocks = visibleBlockEntities;
        visibleBlockEntities = visibleBlockEntitiesSwap;
        visibleBlockEntitiesSwap = tmpBlocks;

        int tmpCount = visibleChunkCount;
        visibleChunkCount = visibleChunkCountSwap;
        visibleChunkCountSwap = tmpCount;

        boolean tmpDirty = dirty;
        dirty = dirtySwap;
        dirtySwap = tmpDirty;
    }
}
