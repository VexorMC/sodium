...
public class ChunkProgram extends GlProgram {
    ...
    public void setup(float modelScale, float textureScale) {
        IrisRenderSystem.uniform1i(this.uBlockTex, 0);
        IrisRenderSystem.uniform1i(this.uLightTex, 1);

        IrisRenderSystem.uniform3f(this.uModelScale, modelScale, modelScale, modelScale);
        IrisRenderSystem.uniform2f(this.uTextureScale, textureScale, textureScale);

        this.fogShader.setup();

        IrisRenderSystem.uniformMatrix4fv(this.uModelViewProjectionMatrix, false, GameRendererContext.getModelViewProjectionMatrix());
    }
}
