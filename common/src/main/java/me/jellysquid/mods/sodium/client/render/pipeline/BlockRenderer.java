...
public class BlockRenderer {
    ...
    private void renderQuad(BlockView world, BlockState state, BlockPos pos, ModelVertexSink sink, Vector3f offset,
        ...
        for (int dstIndex = 0; dstIndex < 4; dstIndex++) {
            ...
            int baseColor = colors != null ? colors[srcIndex] : src.getColor(srcIndex);
            float ao = light.br[srcIndex];

            int color;
            if (useSeparateAo) {
                // AO stored in alpha channel
                color = baseColor & 0x00FFFFFF;
                color |= ((int) (ao * 255.0f)) << 24;
            } else {
                // Vanilla/Sodium default behavior
                if (state.getBlock().isTranslucent()) {
                    color = ColorABGR.mul(baseColor, 1.0f);
                } else {
                    color = ColorABGR.mul(baseColor, ao);
                }
            }
