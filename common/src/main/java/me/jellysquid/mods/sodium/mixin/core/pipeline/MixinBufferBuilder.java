...
public abstract class MixinBufferBuilder implements VertexBufferView, VertexDrain {
    ...
    @Override
    public <T extends VertexSink> T oldium$createSink(VertexType<T> factory1) {
        VertexType<?> factory = factory1;

        if (format == IrisVertexFormats.ENTITY) {
            if (factory1 == VanillaVertexTypes.QUADS) {
                factory = ExtendedQuadVertexType.INSTANCE;
            }
        } else if (format == IrisVertexFormats.TERRAIN) {
            if (factory1 == VanillaVertexTypes.GLYPHS) {
                factory = ExtendedGlyphVertexType.INSTANCE;
            }
        }

        BlittableVertexType<T> blittable = (BlittableVertexType<T>) factory.asBlittable();
