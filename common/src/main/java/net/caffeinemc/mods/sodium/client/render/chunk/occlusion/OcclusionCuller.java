...
public class OcclusionCuller {
    ...
    private void init(WriteQueue<RenderSection> queue) {
        ...
        if (this.origin.getY() < 0) {
            // below the level
            this.outOfWorldRadius = 0;
            this.outOfWorldHeight = 0;
            this.outOfWorldDirection = GraphDirection.DOWN;
        } else if (this.origin.getY() > maxHeight) {
            // above the level
            this.outOfWorldRadius = 0;
            this.outOfWorldHeight = maxHeight;
    ...
    private boolean initOutsideWorldHeight(WriteQueue<RenderSection> queue) {
        var radius = MathHelper.floor(this.searchDistanceRegular / 16.0f);
