...
public class OcclusionCuller {
    ...
    private void init(WriteQueue<RenderSection> queue) {
        ...
        if (origin.getY() < 0) {
            // below the level
            this.initOutsideWorldHeight(queue, 0, GraphDirection.DOWN);
        } else if (origin.getY() > maxHeight) {
            // above the level
            this.initOutsideWorldHeight(queue, maxHeight, GraphDirection.UP);
        } else {
    ...
    private void initOutsideWorldHeight(WriteQueue<RenderSection> queue, int height, int direction) {
        var origin = this.viewport.getChunkCoord();
        var radius = MathHelper.floor(this.searchDistance / 16.0f);
