...
import static net.caffeinemc.mods.sodium.client.render.immediate.ModelCuboid.*;
...
public class EntityRenderer {
    ...
    private static final long[] CUBE_VERTEX_ZW = new long[NUM_CUBE_VERTICES]; // (pos.z, unused)

    private static final int[] CUBE_FACE_NORMAL = new int[NUM_CUBE_FACES];

    public static void renderCuboid(PoseStack.Pose matrices, VertexBufferWriter writer, ModelCuboid cuboid) {
        ...
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            final var vertexCount = emitQuads(vertexBuffer, cuboid);

            if (vertexCount > 0) {
                writer.sodium$push(stack, vertexBuffer, vertexCount, EntityVertex.FORMAT);
            }
        }
    }

    private static int emitQuads(final long buffer, ModelCuboid cuboid) {
        long ptr = buffer;
        ...
        for (int faceIndex = 0; faceIndex < NUM_CUBE_FACES; faceIndex++) {
            ...
            ptr = writeVertex(ptr, positions[elementOffset + 0], textures[elementOffset + 0], packedNormal);
            ptr = writeVertex(ptr, positions[elementOffset + 1], textures[elementOffset + 1], packedNormal);
            ptr = writeVertex(ptr, positions[elementOffset + 2], textures[elementOffset + 2], packedNormal);
            ptr = writeVertex(ptr, positions[elementOffset + 3], textures[elementOffset + 3], packedNormal);
    ...
    private static long writeVertex(long ptr, int vertexIndex, long packedUv, int packedNormal) {
        ...
        MemoryUtil.memPutLong(ptr + 8L, CUBE_VERTEX_ZW[vertexIndex]);
        MemoryUtil.memPutLong(ptr + 12L, packedUv);
        MemoryUtil.memPutInt(ptr + 20L, packedNormal);
    ...
    private static void prepareVertices(PoseStack.Pose matrices, ModelCuboid cuboid) {
        ...
        setVertex(VERTEX_X0_Y0_Z0, c000x, c000y, c000z);
