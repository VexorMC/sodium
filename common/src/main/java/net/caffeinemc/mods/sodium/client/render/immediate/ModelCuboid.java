...
public class ModelCuboid {
    ...
    private static void reverseVertices(int[] vertices, long[] texCoords) {
        for (int faceIndex = 0; faceIndex < NUM_CUBE_FACES; faceIndex++) {
            ...
            int tempV = vertices[vertexOffset];
            vertices[vertexOffset] = vertices[vertexOffset + 3];
            vertices[vertexOffset + 3] = tempV;

            tempV = vertices[vertexOffset + 1];
            vertices[vertexOffset + 1] = vertices[vertexOffset + 2];
            vertices[vertexOffset + 2] = tempV;

            long tempT = texCoords[vertexOffset];
            texCoords[vertexOffset] = texCoords[vertexOffset + 3];
            texCoords[vertexOffset + 3] = tempT;

            tempT = texCoords[vertexOffset + 1];
            texCoords[vertexOffset + 1] = texCoords[vertexOffset + 2];
            texCoords[vertexOffset + 2] = tempT;
        }
    }
