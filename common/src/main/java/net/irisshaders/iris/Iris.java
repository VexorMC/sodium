...
public class Iris {
	...
	public static void onRenderSystemInit() {
		...
		VertexSerializerRegistry.instance().registerSerializer(VertexFormats.ENTITY, IrisVertexFormats.TERRAIN, new EntityToTerrainVertexSerializer());
		...
		VertexSerializerRegistry.instance().registerSerializer(VertexFormats.POSITION_TEXTURE_COLOR, IrisVertexFormats.GLYPH, new GlyphExtVertexSerializer());
		VertexSerializerRegistry.instance().registerSerializer(VertexFormats.ENTITY, IrisVertexFormats.ENTITY, new ModelToEntityVertexSerializer());
	...
	public static void handleKeybinds(Minecraft minecraft) {
		...
		if (reloadKeybind.consumeClick()) {
			...
			if (irisConfig.areDebugOptionsEnabled() && minecraft.player != null && !MinecraftClient.getInstance().isLocalServer()) {
	...
	private static void handleException(Exception e) {
		if (lastDimension != null && irisConfig.areDebugOptionsEnabled()) {
			MinecraftClient.getInstance().setScreen(new DebugLoadFailedGridScreen(MinecraftClient.getInstance().screen, Component.literal(e instanceof ShaderCompileException ? "Failed to compile shaders" : "Exception"), e));
		} else {
			if (MinecraftClient.getInstance().player != null) {
				MinecraftClient.getInstance().player.displayClientMessage(Component.translatable(e instanceof ShaderCompileException ? "iris.load.failure.shader" : "iris.load.failure.generic").append(Component.literal("Copy Info").withStyle(arg -> arg.withUnderlined(true).withColor(
	...
	public static void setDebug(boolean enable) {
		...
		if (enable) {
			...
			GlDebug.enableDebugCallback(MinecraftClient.getInstance().options.glDebugVerbosity, false);
			success = 1;
		}
		...
		if (MinecraftClient.getInstance().player != null) {
			if (IrisPlatformHelpers.getInstance().useELS()) {
				MinecraftClient.getInstance().player.displayClientMessage(Component.translatable("iris.shaders.debug.restartNoDebug"), false);
			} else {
				MinecraftClient.getInstance().player.displayClientMessage(Component.translatable(success != 0 ? (enable ? "iris.shaders.debug.enabled" : "iris.shaders.debug.disabled") : "iris.shaders.debug.failure"), false);
			}

			if (success == 2 && !IrisPlatformHelpers.getInstance().useELS()) {
				MinecraftClient.getInstance().player.displayClientMessage(Component.translatable("iris.shaders.debug.restart"), false);
			}
		}
	}
	...
	public static NamespacedId getCurrentDimension() {
		ClientLevel level = MinecraftClient.getInstance().level;
