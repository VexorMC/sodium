...
public class ShaderStorageBuffer {
	...
	protected void destroy() {
		IrisRenderSystem.bindBufferBase(GL43.GL_SHADER_STORAGE_BUFFER, index, 0);
	...
	public void bind() {
		IrisRenderSystem.bindBufferBase(GL43.GL_SHADER_STORAGE_BUFFER, index, id);
	}
	...
	public void resizeIfRelative(int width, int height) {
		...
		GlStateManager._glBindBuffer(GL43.GL_SHADER_STORAGE_BUFFER, newId);

		// Calculation time
		long newWidth = (long) (width * info.scaleX());
		long newHeight = (long) (height * info.scaleY());
		long finalSize = (newHeight * newWidth) * info.size();
		IrisRenderSystem.bufferStorage(GL43.GL_SHADER_STORAGE_BUFFER, finalSize, 0);
		IrisRenderSystem.clearBufferSubData(GL43.GL_SHADER_STORAGE_BUFFER, GL43.GL_R8, 0, finalSize, GL43.GL_RED, GL43.GL_BYTE, new int[]{0});
		IrisRenderSystem.bindBufferBase(GL43.GL_SHADER_STORAGE_BUFFER, index, newId);
		id = newId;
	}
	...
	public void createStatic() {
		GlStateManager._glBindBuffer(GL43.GL_SHADER_STORAGE_BUFFER, getId());
		IrisRenderSystem.bufferStorage(GL43.GL_SHADER_STORAGE_BUFFER, info.size(), content == null ? 0 : GL46.GL_DYNAMIC_STORAGE_BIT);
		if (content != null) {
			GL46.glBufferSubData(GL43.GL_SHADER_STORAGE_BUFFER, 0, content);
		} else {
			IrisRenderSystem.clearBufferSubData(GL43.GL_SHADER_STORAGE_BUFFER, GL43.GL_R8, 0, info.size(), GL43.GL_RED, GL43.GL_BYTE, new int[]{0});
		}
		bind();
	}
}
