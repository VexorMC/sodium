...
public class FinalPassRenderer {
	...
	public FinalPassRenderer(WorldRenderingPipeline pipeline, ProgramSet pack, RenderTargets renderTargets, TextureAccess noiseTexture, ShaderStorageBufferHolder holder,
		...
		this.lastColorTextureId = MinecraftClient.getInstance().getMainRenderTarget().getColorTextureId();
		this.lastColorTextureVersion = ((Blaze3dRenderTargetExt) MinecraftClient.getInstance().getMainRenderTarget()).iris$getColorBufferVersion();
	...
	public void renderFinalPass() {
		...
		final com.mojang.blaze3d.pipeline.RenderTarget main = MinecraftClient.getInstance().getMainRenderTarget();
