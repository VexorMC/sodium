	...
	public ShaderPack(Path root, Map<String, String> changedConfigs, ImmutableList<StringPair> environmentDefines, boolean isZip) throws IOException, IllegalStateException {
		...
		if (!invalidFeatureFlags.isEmpty()) {
			if (MinecraftClient.getInstance().currentScreen instanceof ShaderPackScreen) {
				Text component = new TranslatableText("iris.unsupported.pack.description", FeatureFlags.getInvalidStatus(invalidFlagList), invalidFeatureFlags.stream()
					.collect(Collectors.joining(", ", ": ", ".")));
				if (SystemUtils.IS_OS_MAC) {
					component = component.append(new TranslatableText("iris.unsupported.pack.macos"));
				}
				MinecraftClient.getInstance().setScreen(new FeatureMissingErrorScreen(MinecraftClient.getInstance().currentScreen, new TranslatableText("iris.unsupported.pack"), component));
			}
