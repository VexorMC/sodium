...
public class FogUniforms {
	...
	public static void addFogUniforms(DynamicUniformHolder uniforms, FogMode fogMode) {
		if (fogMode == FogMode.OFF) {
			...
			uniforms.uniform1i(PER_FRAME, "fogShape", () -> 0);
		}
		...
		uniforms.uniform1f("fogStart", FogHelper::getFogStart, listener -> StateUpdateNotifiers.fogStartNotifier.setListener(listener));

		uniforms.uniform1f("fogEnd", FogHelper::getFogEnd, listener -> StateUpdateNotifiers.fogEndNotifier.setListener(listener));
