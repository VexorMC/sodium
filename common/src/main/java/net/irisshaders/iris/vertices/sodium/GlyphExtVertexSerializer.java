...
public class GlyphExtVertexSerializer implements VertexSerializer {
	...
	private static final int OFFSET_MID_TEXTURE = IrisVertexFormats.GLYPH.getUvIndex(IrisVertexFormats.MID_TEXTURE_ELEMENT.getIndex());
	private static final int OFFSET_LIGHT = 24;
	private static final int OFFSET_NORMAL = IrisVertexFormats.GLYPH.getNormalIndex();
	private static final int OFFSET_TANGENT = IrisVertexFormats.GLYPH.getIndex(IrisVertexFormats.TANGENT_ELEMENT.getIndex());
	...
	public void serialize(long src, long dst, int vertexCount) {
		...
		for (int i = 0; i < vertexCount; i++) {
			...
			if (i != 3) {
				src += VertexFormats.POSITION_TEXTURE_COLOR.getVertexSize();
