...
import com.mojang.blaze3d.systems.BlendFactor;
...
public class DeferredWorldRenderingPipeline implements WorldRenderingPipeline, RenderTargetStateListener  {
	...
	public DeferredWorldRenderingPipeline(ProgramSet programs) {
		...
		int depthTextureId = mainTarget.depthAttachment;
	...
	private RenderCondition getCondition(WorldRenderingPhase phase) {
		...
		switch (phase) {
			...
			case ENTITIES:
				if (GlStateManagerAccessor.getBLEND().srcFactorRGB == BlendFactor.SourceFactor.SRC_ALPHA && GlStateManagerAccessor.getBLEND().dstFactorRGB == BlendFactor.SourceFactor.ONE_MINUS_SRC_ALPHA && GlStateManagerAccessor.getBLEND().srcFactorAlpha == BlendFactor.SourceFactor.ONE && GlStateManagerAccessor.getBLEND().dstFactorAlpha == BlendFactor.SourceFactor.ONE_MINUS_SRC_ALPHA) {
	...
	private final class Pass {
		...
		public void use() {
			...
			if (shadowViewport) {
				...
				RenderSystem.viewport(0, 0, main.viewportWidth, main.viewportHeight);
			}
	...
	private void prepareRenderTargets() {
		...
		int depthTextureId = main.depthAttachment;
		int internalFormat = TextureInfoCache.INSTANCE.getInfo(depthTextureId).getInternalFormat();
		DepthBufferFormat depthBufferFormat = DepthBufferFormat.fromGlEnumOrDefault(internalFormat);
