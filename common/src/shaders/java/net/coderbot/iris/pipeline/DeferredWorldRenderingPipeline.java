...
public class DeferredWorldRenderingPipeline implements WorldRenderingPipeline, RenderTargetStateListener  {
	...
	public DeferredWorldRenderingPipeline(ProgramSet programs) {
		...
		int depthTextureId = ((Blaze3dRenderTargetExt) mainTarget).getIris$depthTextureId();
		int internalFormat = TextureInfoCache.INSTANCE.getInfo(depthTextureId).getInternalFormat();
		DepthBufferFormat depthBufferFormat = DepthBufferFormat.fromGlEnumOrDefault(internalFormat);

		this.renderTargets = new RenderTargets(mainTarget.viewportWidth, mainTarget.viewportHeight, depthTextureId,
		...
		this.table = new ProgramTable<>((condition, availability) -> {
			...
			return cachedPasses.computeIfAbsent(Pair.of(id, availability), p -> {
				ProgramSource source = resolver.resolveNullable(p.first());
	...
	private RenderCondition getCondition(WorldRenderingPhase phase) {
		...
		switch (phase) {
			...
			case ENTITIES:
				if (GlStateManagerAccessor.getBLEND().srcFactorRGB == GL11.GL_SRC_ALPHA && GlStateManagerAccessor.getBLEND().dstFactorRGB == GL11.GL_ONE_MINUS_SRC_ALPHA && GlStateManagerAccessor.getBLEND().srcFactorAlpha == GL11.GL_ONE && GlStateManagerAccessor.getBLEND().dstFactorAlpha == GL11.GL_ONE_MINUS_SRC_ALPHA) {
	...
	private final class Pass {
		...
		public void use() {
			...
			if (shadowViewport) {
				GlStateManager.viewport(0, 0, shadowMapResolution, shadowMapResolution);
			} else {
	...
	private void prepareRenderTargets() {
		...
		int depthTextureId = ((Blaze3dRenderTargetExt) main).getIris$depthTextureId();
		int internalFormat = TextureInfoCache.INSTANCE.getInfo(depthTextureId).getInternalFormat();
		DepthBufferFormat depthBufferFormat = DepthBufferFormat.fromGlEnumOrDefault(internalFormat);

		boolean changed = renderTargets.resizeIfNeeded(mainExt.iris$getDepthBufferVersion(), depthTextureId, main.viewportWidth,
	...
	public void onBindTexture(int id) {
		if (shouldBindPBR && isRenderingWorld) {
			...
			currentNormalTexture = pbrHolder.getNormalTexture().getGlId();
			currentSpecularTexture = pbrHolder.getSpecularTexture().getGlId();
