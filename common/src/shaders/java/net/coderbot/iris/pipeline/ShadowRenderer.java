...
public class ShadowRenderer {
	...
	private void renderEntities(LevelRendererAccessor levelRenderer, Frustum frustum, PoseStack modelView, double cameraX, double cameraY, double cameraZ, float tickDelta) {
		...
		for (Entity entity : getLevel().getLoadedEntities()) {
			if (!dispatcher.shouldRender(entity, new CullingCameraView(frustum), cameraX, cameraY, cameraZ)) {
				continue;
			}
	...
	private void renderPlayerEntity(Frustum frustum, PoseStack modelView, double cameraX, double cameraY, double cameraZ, float tickDelta) {
        EntityRenderDispatcher dispatcher = MinecraftClient.getInstance().getEntityRenderManager();
		...
		if (!dispatcher.shouldRender(player, new CullingCameraView(frustum), cameraX, cameraY, cameraZ)) {
			return;
		}
	...
	public void renderShadows(LevelRendererAccessor levelRenderer) {
		...
		levelRenderer.invokeSetupRender(MinecraftClient.getInstance().getCameraEntity(), CapturedRenderingState.INSTANCE.getTickDelta(), new CullingCameraView(terrainFrustumHolder.getFrustum()), ((GameRendererAccessor)MinecraftClient.getInstance().gameRenderer).getFrameCount(), false);
		...
		if (shouldRenderTerrain) {
			levelRenderer.invokeRenderChunkLayer(RenderLayer.SOLID, CapturedRenderingState.INSTANCE.getTickDelta(), 2, MinecraftClient.getInstance().getCameraEntity());
			levelRenderer.invokeRenderChunkLayer(RenderLayer.CUTOUT, CapturedRenderingState.INSTANCE.getTickDelta(), 2, MinecraftClient.getInstance().getCameraEntity());
			levelRenderer.invokeRenderChunkLayer(RenderLayer.CUTOUT_MIPPED, CapturedRenderingState.INSTANCE.getTickDelta(), 2, MinecraftClient.getInstance().getCameraEntity());
		}
        ...
        GL11.glLoadMatrixf(modelView.last().pose().get(new float[16]));
		...
		if (shouldRenderEntities) {
			renderEntities(levelRenderer, entityShadowFrustum, modelView, cameraX, cameraY, cameraZ, tickDelta);
		} else if (shouldRenderPlayer) {
			renderPlayerEntity(entityShadowFrustum, modelView, cameraX, cameraY, cameraZ, tickDelta);
		}
		...
		if (shouldRenderTranslucent) {
            levelRenderer.invokeRenderChunkLayer(RenderLayer.TRANSLUCENT, CapturedRenderingState.INSTANCE.getTickDelta(), 2, MinecraftClient.getInstance().getCameraEntity());
		}
	...
	private String getEntitiesDebugString() {
		return (shouldRenderEntities || shouldRenderPlayer) ? (renderedShadowEntities + "/" + MinecraftClient.getInstance().world.getLoadedEntities()) : "disabled by pack";
	}
