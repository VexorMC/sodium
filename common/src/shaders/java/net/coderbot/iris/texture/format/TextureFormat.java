...
public interface TextureFormat {
	...
	default void setupTextureParameters(PBRType pbrType, Texture texture) {
		if (!canInterpolateValues(pbrType)) {
			int minFilter = IrisRenderSystem.getTexParameteri(texture.getGlId(), GL11.GL_TEXTURE_2D, GL11.GL_TEXTURE_MIN_FILTER);
			...
			IrisRenderSystem.texParameteri(texture.getGlId(), GL11.GL_TEXTURE_2D, GL11.GL_TEXTURE_MIN_FILTER, mipmap ? GL11.GL_NEAREST_MIPMAP_NEAREST : GL11.GL_NEAREST);
			IrisRenderSystem.texParameteri(texture.getGlId(), GL11.GL_TEXTURE_2D, GL11.GL_TEXTURE_MAG_FILTER, GL11.GL_NEAREST);
		}
	}
