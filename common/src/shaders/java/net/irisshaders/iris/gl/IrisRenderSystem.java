...
public class IrisRenderSystem {
	...
	public static int getMaxImageUnits() {
		if (GL.getCapabilities().OpenGL42 || GL.getCapabilities().GL_ARB_shader_image_load_store) {
			return GL15.glGetInteger(GL42C.GL_MAX_IMAGE_UNITS);
		} else if (GL.getCapabilities().GL_EXT_shader_image_load_store) {
			return GL15.glGetInteger(EXTShaderImageLoadStore.GL_MAX_IMAGE_UNITS_EXT);
		} else {
			return 0;
		}
	}
	...
	public static void setShadowProjection(Matrix4f shadowProjection) {
		backupProjection = new Matrix4f(Camera.PROJECTION_MATRIX);
        GL11.glMatrixMode(GL11.GL_PROJECTION);
        try (MemoryStack stack = MemoryStack.stackPush()) {
            GL11.glLoadMatrixf(shadowProjection.get(stack.callocFloat(16)));
        }
	}
	...
	public static void restorePlayerProjection() {
        GL11.glMatrixMode(GL11.GL_PROJECTION);
        try (MemoryStack stack = MemoryStack.stackPush()) {
            GL11.glLoadMatrixf(backupProjection.get(stack.callocFloat(16)));
        }
	...
	public static void backupAndDisableCullingState(boolean b) {
		cullingState = SodiumClientMod.options().performance.smartCull;
        SodiumClientMod.options().performance.smartCull = SodiumClientMod.options().performance.smartCull && !b;
	}

	public static void restoreCullingState() {
        SodiumClientMod.options().performance.smartCull = cullingState;
		cullingState = true;
	}
